...
public class RunningAverage {
  ...
  protected static void getRatingAverageTable(KStream<Long, Rating> ratings,
    ...
    ratingsById
            ...
            // TODD: reduce buffering events
            .aggregate(new Initializer<List<Double>>() {
              @Override
              public List<Double> apply() {
                return new ArrayList<>();
              }
            }, new Aggregator<Long, Double, List<Double>>() {
              @Override
              public List<Double> apply(Long key, Double value, List<Double> aggregate) {
                aggregate.add(value);
                return aggregate;
              }
            }, Materialized.as("aggregated-stream-store").withValueSerde(new ListSerde(Double()))
)            .suppress(Suppressed.untilWindowCloses(unbounded()))
            .toStream().foreach((key, value) -> {
                      System.out.println("================");
                      System.out.println(((List) value).stream().
                              map(Object::toString).
